...
for (i in seedMin:seedMax){
  ...
  percWW = pnorm(resultsWW,mean=mn,sd=sd)
  percMW = pnorm(resultsMW,mean=mn,sd=sd)
  wealthW = c(percWW, percMW)
  wealthW[is.na(wealthW)] <- 0
  #Gini Coefficient male-optimal
  percMM = pnorm(resultsMM,mean=mn,sd=sd)
  percWM = pnorm(resultsWM,mean=mn,sd=sd)
...
ggplot(data = DF, aes(x = avg)) + 
  ...
  #geom_density(data = filter(DF, algorithm == "male-optimal" & sex == "men"), fill = "#2595FF", color = NA, alpha = 0.3) +
    ...
    geom_density(data = filter(DF, algorithm == "female-optimal" & sex == "women"), fill = "#FE18AC", color = NA, alpha = 0.3) +
...
plotMen <- ggplot(data = dfPlot, aes(x=value, fill=variable)) + 
  geom_vline(data=dfPlot, aes(xintercept=uMWav), linetype="solid", size=1, colour = "#2595FF") +
...
plotWomen <- ggplot(data = dfPlot, aes(x=value, fill=variable)) + 
  geom_vline(data=dfPlot, aes(xintercept=uWWav), linetype="solid", size=1, colour = "#FE18AC") +
  geom_density(alpha=1/(repetitions+1),show.legend=FALSE, colour=NA) +
