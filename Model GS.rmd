...
### Aggregate Gender Satisfaction Chart
...
ggplot(data = DF, aes(x = avg, fill=sex)) + 
  ...
  geom_bar(data = filter(DF, algorithm == "female-optimal"),show.legend=FALSE, color = NA, alpha = 0.3, position="identity") +
  scale_fill_manual(values=c("#2595FF", "#FE18AC")) +
...
### Individual Matching Satisfaction across all iterations
...
title <- ggdraw() + 
  draw_label("Individual Matching Satisfaction", fontface='bold', size = 16, x=0.05, y=0.5, hjust = 0, vjust=0) +
  draw_label(paste("Set size = ", setSize," || men/women (in %) = ", nmen/setSize*100, ":", nwomen/setSize*100),size=10, x = 0.05, y=0.24, hjust=0, vjust=0) + 
  draw_label(paste("Preference structure individual/collective/homolog (in %) = ", round(wI*100), "/", round(wC * 100), "/", round(wH * 100), sep = ""),size=10, x = 0.05, y=0.05, hjust=0, vjust=0)
